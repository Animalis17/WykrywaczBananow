...
unit_of_measure = SDS018.UnitsOfMeasure.MassConcentrationEuropean
for arg in range(len(sys.argv) - 1, 0, -1):
    if sys.argv[arg][0:2] == '-d':
        ...
        if sys.argv[arg + 1] == '0':
            ...
            unit_of_measure = SDS018.UnitsOfMeasure.ParticelConcentrationImperial
        else:
...
def printValues(timing, values, unit_of_measure):
    if unit_of_measure == SDS018.UnitsOfMeasure.MassConcentrationEuropean:
        unit = 'µg/m³'
    else:
        unit = 'pcs/0.01cft'
...
sensor = SDS018(sys.argv[1], timeout=timeout, unit_of_measure=unit_of_measure)
# raise KeyboardInterrupt
# Now we have some details about it
print("SDS018 sensor info:")
...
try:
    ...
    for a in range(cycles):
        ...
        sensor.workstate = SDS018.WorkStates.Measuring
        ...
        sensor.workstate = SDS018.WorkStates.Sleeping
        time.sleep(5)
